...
class StudioCapturePro(ctk.CTk):
    def __init__(self):
        ...
        # Asynchronous Audio Matrix Storage Buffers
        self.sys_audio_data = []
        self.mic_audio_data = []
        
        # Adaptive Target Specification Fields
        self.sys_target_rate = 44100
        self.mic_target_rate = 44100
    ...
    def _assemble_fluid_interface(self) -> None:
        ...
        self.ui_fps_badge = ctk.CTkLabel(self.header_panel, text="UNIVERSAL ADAPTIVE AUDIO ENGINE • 90FPS", font=("Roboto", 10, "bold"), text_color="#515666")
        ...
        self.switch_sys_audio = ctk.CTkSwitch(self.control_panel, text="Capture System Sound (Speakers/Headset Auto)", font=("Roboto", 12), command=self._update_preferences, text_color="#a0a5b5")
        self.switch_sys_audio.select()
        self.switch_sys_audio.grid(row=0, column=1, padx=25, pady=18, sticky="w")
        
        self.switch_mic_audio = ctk.CTkSwitch(self.control_panel, text="Include Microphone Voice (Auto-Target Active)", font=("Roboto", 12), command=self._update_preferences, text_color="#a0a5b5", progress_color="#00bcd4")
    ...
    def _update_preferences(self) -> None:
        ...
        self.record_system_sound = bool(self.switch_sys_audio.get())
        self.record_mic_voice = bool(self.switch_mic_audio.get())
    ...
    def execute_start_pipeline(self) -> None:
        ...
        # Fire isolated frame video encoder process
        self._launch_hardware_encoder_process()
        
        # Dynamically query and attach the active device pipelines on thread initialization
    ...
    def _system_audio_capture_loop(self):
        """Discovers the active default speaker dynamically and adapts to its hardware sample rate profile."""
        try:
            active_speaker = sc.default_speaker()
            self.sys_target_rate = int(active_speaker.samplerate) if hasattr(active_speaker, 'samplerate') else 48000
            
            # Query the corresponding lookback node directly from the windows audio server
            loopback_device = sc.get_microphone(id=active_speaker.name, include_loopback=True)
            print(f"[Dynamic Capture] System Loopback established on \"{loopback_device.name}\" at {self.sys_target_rate}Hz.")
            
            with loopback_device.recorder(samplerate=self.sys_target_rate) as loopback:
                while not self.stop_signal_event.is_set():
                    data = loopback.record(numframes=1024)
                    self.sys_audio_data.append(data)
        except Exception as e:
            print(f"[Dynamic Capture Error] System audio tracking dropped: {e}")

    def _microphone_capture_loop(self):
        """Discovers the active hardware microphone input dynamically and isolates its sample profile."""
        try:
            # Query all hardware inputs to lock onto an active USB headset mic first, falling back onto motherboard lines
            mic_device = None
            for hardware_input in sc.all_microphones():
                if "usb audio" in hardware_input.name.lower():
                    mic_device = hardware_input
                    break
            if not mic_device:
                mic_device = sc.default_microphone()
                
            self.mic_target_rate = int(mic_device.samplerate) if hasattr(mic_device, 'samplerate') else 48000
            print(f"[Dynamic Capture] Microphone Input established on \"{mic_device.name}\" at {self.mic_target_rate}Hz.")
            
            with mic_device.recorder(samplerate=self.mic_target_rate) as mic:
                while not self.stop_signal_event.is_set():
                    data = mic.record(numframes=1024)
                    self.mic_audio_data.append(data)
        except Exception as e:
            print(f"[Dynamic Capture Error] Microphone voice tracking dropped: {e}")
        ...
        if self.ffmpeg_process_handle:
            for line in iter(self.ffmpeg_process_handle.stderr.readline, b''):
                ...
                if "error" in decoded_line.lower() or "fail" in decoded_line.lower() or "invalid" in decoded_line.lower():
                    print(f"[FFmpeg Warning] {decoded_line}")
    ...
    def _teardown_subprocess_worker(self) -> None:
        ...
        # Matrix Formatting: Downmix multi-channel fields to universal Stereo format natively via numpy vectors
        if self.record_system_sound and len(self.sys_audio_data) > 0:
            try:
                sys_matrix = np.concatenate(self.sys_audio_data, axis=0)
                if len(sys_matrix.shape) > 1 and sys_matrix.shape[1] > 2:
                    sys_matrix = np.mean(sys_matrix, axis=1, keepdims=True)
                    sys_matrix = np.concatenate([sys_matrix, sys_matrix], axis=1)
                sf.write(self.tmp_sys_audio_path, sys_matrix, self.sys_target_rate)
                has_sys = True
            except Exception as e: print(f"[Compiler Error] System processing exception: {e}")

        if self.record_mic_voice and len(self.mic_audio_data) > 0:
            try:
                mic_matrix = np.concatenate(self.mic_audio_data, axis=0)
                if len(mic_matrix.shape) > 1 and mic_matrix.shape[1] > 2:
                    mic_matrix = np.mean(mic_matrix, axis=1, keepdims=True)
                    mic_matrix = np.concatenate([mic_matrix, mic_matrix], axis=1)
                sf.write(self.tmp_mic_audio_path, mic_matrix, self.mic_target_rate)
                has_mic = True
            except Exception as e: print(f"[Compiler Error] Mic processing exception: {e}")
    ...
    def _restore_and_reset_ui(self) -> None:
        ...
        self.switch_sys_audio.configure(state="normal")
        self.switch_mic_audio.configure(state="normal")
        
        self.ui_io_label.configure(text=f"Repository Location: {self.save_directory}")
        self.deploy_toast_notification("✓ Layout rendered successfully at 90FPS! Files synchronized cleanly.")
